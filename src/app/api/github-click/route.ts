...
const GITHUB_CLICK_EVENTS_KEY = 'github_click_events';
const GITHUB_CLICK_EVENTS_KEEP_DAYS = 60;

async function upstashMultiExec(commands: any[][]) {
  if (!UPSTASH_REDIS_REST_URL || !UPSTASH_REDIS_REST_TOKEN) {
    console.error('GITHUB_CLICK upstash env vars missing');
    return null;
  }
  ...
  if (!res.ok) {
    ...
    console.error(`GITHUB_CLICK upstash error ${res.status}: ${text}`);
    return null;
  }
  return res.json();
}
...
export async function POST(req: Request) {
  ...
  // Source identifies WHICH GitHub button: 'profile' (hero/social) or a
  // project slug/name.
  const source = sanitize(params.get('source'), 60) || 'unknown';
  const target = (params.get('target') ?? '').slice(0, 200); // full GitHub URL

  // Same client-sent fields as cv-click for consistency.
  ...
  console.log(
    `GITHUB_CLICK source=${source} target=${target} country=${country}`
  );
  ...
  const minKeepMs =
    nowMs - GITHUB_CLICK_EVENTS_KEEP_DAYS * 24 * 60 * 60 * 1000;

  await upstashMultiExec([
    ['ZADD', GITHUB_CLICK_EVENTS_KEY, nowMs, event],
    ['ZREMRANGEBYSCORE', GITHUB_CLICK_EVENTS_KEY, 0, minKeepMs],
  ]);
